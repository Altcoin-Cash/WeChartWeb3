...
class Router {
  ...
  async getRouter(routerAdd, pairAdd) {
    ...
    if (!routerCached) {
      ...
      if (!router) {
        ...
        this.bulk.bulk_normal.setNewDocument(
          routerAdd,
          EnumBulkTypes.ROUTERS,
          routerInfos
        );
        //console.log(`\t\t[ROUTER] ${routerAdd} -> Fee ${fee}`);
      } else if (!router.fee) {
        // if somehow the router was saved without the fee
        let fee = await this.getFee(routerAdd, pairAdd);
        fee = parseInt(fee / 10 ** 3) / 10 ** 4;
        this.bulk.bulk_normal.setTokenBulkSet(
          routerAdd,
          EnumBulkTypes.ROUTERS,
          "fee",
          fee
  ...
  async getFeePancakeSimilar(routerAdd) {
    let routerContract = await new this.web3.eth.Contract(
      EnumAbi[EnumChainId.ALT].ROUTERS.PANCAKE,
      routerAdd
    );
    let factory = await routerContract.methods.factory().call();
    let fee = await routerContract.methods
      .getAmountOut(10000000, 100000000000, 100000000000)
      .call();
    return parseInt(fee);
  }
  async getFeeBiswapSimilar(routerAdd, pairAdd) {
    ...
    if (!pairAdd) return -1;
    let routerContractBiswap = await new this.web3.eth.Contract(
      EnumAbi[EnumChainId.ALT].ROUTERS.BISWAP,
      routerAdd
    );
    let factory = await routerContractBiswap.methods.factory().call();
    let pairContract = await new this.web3.eth.Contract(
      EnumAbi[EnumChainId.ALT].PAIR.BISWAP,
      pairAdd
    );
    let pairFee = parseInt(await pairContract.methods.swapFee().call());
    let fee = await routerContractBiswap.methods
      .getAmountOut(10000000, 100000000000, 100000000000, pairFee)
      .call();
    return parseInt(fee);
  }
  async getRouters() {
