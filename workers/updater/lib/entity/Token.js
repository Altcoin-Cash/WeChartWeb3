...
class Token {
  ...
  async getToken(token) {
    ...
    if (tokenInfo && tokenInfo.notFound)
      if (Date.now() - tokenInfo.date < 1000 * 60)
        // scrape a not found token only after one minute
        searchOnDb = false;
    ...
    if (searchOnDb && (!tokenInfo || tokenInfo.notFound)) {
      ...
      if (!tokenInfo) {
        let token_contract = await new this.web3.eth.Contract(
          EnumAbi[EnumChainId.ALT].TOKEN,
          token
        );
        let token_decimals;
        let name;
        ...
        try {
          token_decimals = parseInt(
            await token_contract.methods.decimals().call()
          );
          name = await token_contract.methods.name().call();
          supply =
            parseInt(await token_contract.methods.totalSupply().call()) /
            10 ** token_decimals;
    ...
    if (tokenInfo && !tokenInfo.burned)
      tokenInfo.burned =
        (await this.getBurned(token)) / 10 ** tokenInfo.decimals;

    return tokenInfo;
  }
  async getBurned(token) {
    try {
      let tokenContract = await new this.web3.eth.Contract(
        EnumAbi[EnumChainId.ALT].PAIR.PANCAKE,
        token
      );
      let zeroAddAmount = await tokenContract.methods
        .balanceOf("0x0000000000000000000000000000000000000000")
        .call();
      let burnAddAmount = await tokenContract.methods
